...
- Home surfaces matching Slack: **Activity**, **Later**, **Threads**, **Direct Messages**, **Drafts**, **Unreads**
...
Everything here was captured from the official client (HAR / browser protocol). Uncaptured surfaces (create channel, invite members, mentions-only notification prefs, a starred-**items** inbox) are **not** invented — they are omitted.
...
- **Star** — `*` stars a **channel** (Slack Starred sidebar section). Actions menu stars a **message** (`stars.add` with timestamp). There is no starred-items *inbox*.
...
Same non-goals as upstream, plus surfaces we have **not captured** and will not invent:

- Huddles, Slack Connect, Workflow Builder, Canvas, Lists, Slack AI, slash commands, custom emoji management, animated reactions
- Create channel / invite members (Join / Leave already exist)
- Mentions-only notification prefs (`users.prefs.set` name besides `muted` not captured)
- Starred-**items** message inbox (channel Starred section exists via `stars.list` type=channel)
- Unreads “recommended / scientifically” sort and persisted section-filter prefs (algorithm / extra pref values unknown)
...
- [Installation](wiki/Installation.md) — prebuilt binaries, Go install, build from source
