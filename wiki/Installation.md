...
Current release: **v0.19.0** (first fork tag: v0.17.0). Homebrew formula pins that tag; `--HEAD` tracks `main`. Go: `@v0.19.0` or `@latest`.
...
That auto-taps [agustif/homebrew-tap](https://github.com/agustif/homebrew-tap) and builds **v0.19.0** from source (or `main` with `--HEAD`).
...
In-tree `flake.nix` builds this source (`version = "0.19.0"`). Not a published nixpkgs package.
...
GitHub Release **v0.19.0** includes Windows zips from GoReleaser. Or clone and build:
